...
def clear_mappers():
    ...
    well. As such, :func:`.clear_mappers` is only for usage in my_test suites
    that re-use the same classes with different mappings, which is itself an
    extremely rare use case - the only such use case is in fact SQLAlchemy's
    own my_test suite, and possibly the my_test suites of other ORM extension
    libraries which intend to my_test various combinations of mapper construction
