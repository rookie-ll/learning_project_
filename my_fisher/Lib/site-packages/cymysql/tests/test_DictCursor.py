...
class TestDictCursor(base.PyMySQLTestCase):
    ...
    def test_DictCursor(self):
        #all assert my_test compare to the structure as would come out from MySQLdb
        ...
        try:
            ...
            # same my_test again but iterate over the
            c.execute("SELECT * from dictcursor where name='bob'")
            for r in c:
            ...
            #same my_test again but do a list comprehension
            c.execute("SELECT * from dictcursor")
            r = [x for x in c]
