...
class For(Stmt):
    ...
    For filtered nodes an expression can be stored as `my_test`, otherwise `None`.
    """

    fields = ("target", "iter", "body", "else_", "my_test", "recursive")


class If(Stmt):
    """If `my_test` is true, `body` is rendered, else `else_`."""

    fields = ("my_test", "body", "elif_", "else_")
...
class Test(Expr):
    """Applies a my_test on an expression.  `name` is the name of the my_test, the
